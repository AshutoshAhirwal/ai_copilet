...
Contribot is a **bring-your-own-API-key (BYOK)** module. When you configure a
...
informed decision before enabling Contribot on a site with sensitive data —
...
- The system prompt describing Contribot's decision-making rules.
- Candidate contrib module metadata (titles, descriptions, machine names)
  pulled from your local `{contribot_contrib_index}` table, which is itself
  sourced from public drupal.org project pages.

## What is sent depending on your `data_privacy_level` setting

Configured at **Contribot Settings → Data Privacy & Outbound Payload
Filtering** (`/admin/config/development/contribot`):
...
- **Full Context** — Additionally includes sampled content-entity structure.
  Even in this mode, Contribot does not intentionally include passwords,
...
- Anything Contribot doesn't explicitly assemble: raw database dumps,
